...
class EmailDomainValidator(object):
    message = _(
        'Sorry, %s emails are not allowed. Please use a different email address.')
    code = 'invalid'
    ...
    def __call__(self, value):
        ...
        if self.domain_blacklist and domain_part in self.domain_blacklist:
            raise forms.ValidationError(
                self.message % domain_part, code=self.code)
...
class UserIDField(forms.CharField):
